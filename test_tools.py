...
async def test(name, fn, *args, min_length=10, **kwargs):
    try:
...
async def main():
    ...
    # Test 1: Open calls
    r1 = await test("search_open_calls (no filter)", search_open_calls, limit=3)

    # Test 2: Planned calls
    r2 = await test("search_planned_calls", search_planned_calls, limit=3)

    # Test 3: Approved applications
    r3 = await test("search_approved_applications", search_approved_applications, limit=3)
    ...
    if call_id:
        await test(f"get_call_detail (id={call_id})", get_call_detail, call_id)
    else:
    ...
    if app_id:
        await test(f"get_application_detail (id={app_id})", get_application_detail, app_id)
    else:
    ...
    # Test 6: Projects
    r6 = await test("search_projects (in realisation)", search_projects, in_realisation=True, limit=3)

    # Test 7: Project detail
    proj_id = None
    if r6:
        try:
    ...
    if proj_id:
        await test(f"get_project_detail (id={proj_id})", get_project_detail, proj_id)
    else:
    ...
    await test("get_programme_structure", get_programme_structure)
